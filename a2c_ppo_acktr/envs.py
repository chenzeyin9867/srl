...
def make_passive_haptics_env(seed,
                             ...
                             num_frame_stack=10,
                             random=False):
    passive_haptics_env = PassiveHapticsEnv(num_frame_stack, random)
    return passive_haptics_env


class PassiveHapticsEnv(object):
    def __init__(self, num_frame_stack, random=False):
        ...
        self.v_direction = 0
        self.p_direction = 0
        self.obj_x = WIDTH_ALL / 2
        self.obj_y = HEIGHT_ALL / 2
        self.obj_x_p = WIDTH / 2
        self.obj_y_p = HEIGHT / 2
        self.obj_d = PI / 4.0
        self.x_physical = 0
        self.y_physical = 0
        self.x_virtual = 0
        self.y_virtual = 0
        self.turn_flag = np.random.randint(STEP_LOW, STEP_HIGH)
        self.time_step = 0
        ...
        #print(random)
        if not random:
            self.pas_path_file = np.load('./Dataset/new_passive_haptics_path_30.npy', allow_pickle=True)
    ...
    def get_obs(self):
        ...
        return [(self.x_physical+DELTA_X-(WIDTH_ALL)/2)/(WIDTH_ALL/2), (self.y_physical+DELTA_Y-(HEIGHT_ALL/2))/(HEIGHT_ALL/2), (self.p_direction) / (PI),
                ...
                (self.obj_x_p+DELTA_X-(WIDTH_ALL)/2)/(WIDTH_ALL/2), (self.obj_y_p+DELTA_Y-(HEIGHT_ALL)/2)/(HEIGHT_ALL/2), (self.obj_d)/(PI),
    ...
    def step(self, action):
        ...
        if not signal:  # reset the env
            ...
            # print("r3:", (sum(self.r3)/len(self.r3)), "std:", np.std(self.r3), end='t')
    ...
    """
    def init_eval_state(self, evalType=0):
        if evalType == 2:
    ...
    def step_specific_path(self, actor_critic, ind, ep=None, evalType=1):
    ...
    def final_reward(self):
        return (10 - distance(self.x_physical, self.y_physical, self.obj_x_p, self.obj_y_p))
    def step_specific_path_nosrl(self, ind, evalType=0):
        x_l = []
        y_l = []
        collide = 0
        self.v_path = self.pas_path_file[ind]
    ...
    def get_reward(self):
        ...
        r2, d2 = self.get_reward_distance()
    ...
    def get_reward_distance(self):
        ...
        d1_ratio = distance((self.x_physical+DELTA_X)/WIDTH_ALL, (self.y_physical+DELTA_Y)/HEIGHT_ALL, self.obj_x/WIDTH_ALL, (self.obj_y)/HEIGHT_ALL)
        ...
        # delta_distance_ratio = abs(d1_ratio-d2_ratio) * np.sqrt(2)
        delta_distance_ratio = abs(d1_ratio-d2_ratio) * np.sqrt(2) * np.exp(- 5 * d2_ratio)
        ...
        return toTensor(delta_distance_ratio), d2_ratio
    ...
    def get_reward_angle(self):
        ...
        vec1 = np.array([self.obj_x-DELTA_X-self.x_physical, self.obj_y-DELTA_Y-self.y_physical])
